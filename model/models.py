...
class TransformerModel(pl.LightningModule):
    ...
    def __init__(self, lr=1e-3, d_model=1280, num_heads=8, hidden_size = 1, dropout = 0.0):
        super().__init__()
    ...
    def forward(self, x, t):
        ...
        pos_enc = positional_encoding(x.shape[1], x.shape[2]).unsqueeze(0) #unsqueeze broadcasts across batch
        pos_enc = pos_enc.to(self.device)

        time_enc = time_encoding(t, x.shape[1], x.shape[2]).unsqueeze(0)
    ...
    def training_step(self, x1, batch_idx):
        ...
        #Get noised version
        x_t, x0 = interpolate(x1, t, mask = mask, device= self.device)
