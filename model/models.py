...
class ProjectDownTransformerModel(pl.LightningModule):

    def __init__(self, 
                 lr=1e-3, 
                 d_input=1280, #input data dimension
                 d_flow = 32,
                 n_blocks=1, 
                 num_heads=8, 
                 hidden_size = 1, 
        ...
        self.project_down = nn.Linear(d_input, d_flow)
        self.project_up = nn.Linear(d_flow, d_input)
        self.lr = lr
        self.embedding_scale = embedding_scale
        self.source_scale = source_scale
        self.d_flow = d_flow
    ...
    def forward(self, x, t):
        ...
        mask = mask[:,:,:self.d_flow] #padding is added in the 2nd dimension, all elements of the 3rd are the same
                                      #This reshapes to the dimension of the flow, where t
        ...
        x = x*mask #why not help the model out? It should figure out that 0s are padding, but let's lend a hand
        x = self.project_up(x)
        return x
